...
exports.createAlert = async (req, res) => {
  try {
    const { reporterName, reporterPhone, type, description, location, triageLevel, triageResponses } = req.body;
...
exports.getDepartmentAlerts = async (req, res) => {
  try {
    ...
    if (deptType === 'police') {
      query = { $or: [{ assignedDepartment: 'police' }, { type: 'Crime' }, { type: 'Accident' }] };
    } else if (deptType === 'fire') {
      query = { $or: [{ assignedDepartment: 'fire' }, { type: 'Fire' }] };
    } else if (deptType === 'ambulance') {
      query = { $or: [{ assignedDepartment: 'ambulance' }, { type: 'Medical' }, { type: 'Accident' }] };
    }
